...
import {createNeighbors} from 'shared/algorithms/utils/neighbors';
import {createVisited} from 'shared/algorithms/utils/visited';
...
class Searcher {
  ...
  private aggregateByLevel(nodes: NodeFrontier[]) {
    ...
    for (const node of nodes) arr2D[node!.cost]?.push(node!.state);
    return arr2D;
  }
